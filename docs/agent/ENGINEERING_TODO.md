...
- **Dependencies:** Checkout flow. **Status:** [ ]

## High
Important architectural or functional problems.

- H1. Self-service payment success: `PUT /api/payments/{id}/checkOut-succeed` accepts client-supplied TransactionId with no gateway verification → free enrollments. Restrict to admin/gateway. (`PaymentsController.cs`) [ ]
- H2. `MarkAsComplete`/certificate have no enrollment check → any user can complete any lesson/course. (`clsProgress.cs`, `ProgressController.cs`) [ ]
- H3. Audit IP field stores `HttpContext.Connection.Id` (GUID) instead of remote IP in all controllers. [ ]
- H4. Courses/lessons cannot be published via API (only bundles have publish endpoints) → unsellable. [ ]
- H5. Order state machine: `complete` bypasses payment and then blocks payment forever; `cancel` orphans pending payments. [ ]
- H6. `SP_AddNewItemToOrder` adds unpublished products with NULL PriceAtPurchase. [ ]
- H7. Idempotency decorative: server-side fresh GUID per call; retries create duplicate payments. [ ]
...
- M3. Route/param mismatches: `GET/DELETE /api/courses/{id}/instructors` bind `courseId` vs route token `id` → 400. (`CoursesController.cs`) [ ]
...
- M5. `GetLessonById` uses only first role claim; multi-role users misrouted. (`LessonsController.cs`) [ ]
...
- M13. Rate limiting only on login/refresh; registration, email-exists, certificate generation unlimited. [ ]
...
- M17. 429 body-writer middleware writes login-specific message for all rate-limited endpoints, after response may have started. [ ]
